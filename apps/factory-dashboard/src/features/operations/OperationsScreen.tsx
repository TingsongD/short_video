...
export function OperationsScreen({section,data,selected,reviewer,act}:Props){
  ...
  return <section><h2>{section}</h2>
    {section==='Budgets'&&<><p>Each provider keeps its own unit. Recording a ceiling does not buy credits.</p>
      ...
      <h3>Reusable spending budgets</h3><table><thead><tr><th>Budget</th><th>Unit</th><th>Ceiling</th><th>Available</th></tr></thead><tbody>{(data.budgets||[]).filter(selectableBudget).map(b=><tr key={b.id}><td>{b.id}</td><td>{b.unit}</td><td>{b.cap_amount}</td><td>{b.retired?'Retired after restore':b.available}</td></tr>)}</tbody></table>
      <details><summary>Per-run cumulative guardrails</summary><ul>{(data.budgets||[]).filter(b=>b.classification==='run_guardrail').map(b=><li key={b.id}>{b.id}: {b.available} of {b.cap_amount} {b.unit} remains; it cannot fund another run.</li>)}</ul></details>
      <details><summary>Internal authorization ceilings — not reusable funding</summary><ul>{(data.budgets||[]).filter(b=>String(b.id).startsWith('authority:')).map(b=><li key={b.id}>{b.id}: {b.cap_amount} {b.unit}</li>)}</ul></details>
      <h3>Usage evidence</h3><p>Estimates are not confirmed charges. Unresolved holds include historical unknown outcomes and retain their evidence and do-not-retry decisions. Shared budget rows overlap: do not add them together.</p>
      <table><thead><tr><th>Budget / unit</th><th>Pending estimates</th><th>Estimated usage</th><th>Provider-confirmed usage</th><th>Native quoted usage</th><th>Unresolved holds</th></tr></thead><tbody>{(data.budgets||[]).map(b=><tr key={b.id}><td>{b.id} / {b.unit}</td>{['pending_estimates','estimated_usage','confirmed_usage','quoted_usage','unresolved_holds'].map(k=><td key={k}>{b[k] ?? 'Unavailable'}</td>)}</tr>)}</tbody></table>
    ...
    {(['Research','Audio','Products','Analysis'].includes(section))&&<><h3>Reviewed effect plans</h3><select aria-label="Effect plan" value={effect?.id||''} onChange={e=>{setEffect(plans.find(p=>p.id===e.target.value)||null);setAuthority('');}}><option value="">Choose a quote</option>{plans.map(p=><option key={p.id} value={p.id}>{p.id}</option>)}</select>
      ...
      <label>Funded budget<select value={budgetId} onChange={e=>setBudget(e.target.value)}><option value="">Choose funding</option>{(data.budgets||[]).filter(selectableBudget).map(b=><option key={b.id}>{b.id}</option>)}</select></label>

...
EXPECTED = {
    ...
    "F21-M01": "9 unique pics, 4 selections, shared billed once",
    "F21-M02": "reject C shot → only C chain blocks; shared intact",
    "F21-M03": "restart mid-downloads resumes; no resubmits",
    "F21-M04": "over-limit → declared split or validated manual coverage",
    "F22-M01": "4×900f variants, bound assets, real check/plan clean",
    ...
    "F24-M01": "4 finals: evidence packs + verdicts; tech/creative split",
    "F24-M02": "entity defect, wrong product, shifted join — all located",
    "F24-M03": "frame + gain leaks outside declared region both caught",
    "F24-M04": "altered bytes stale the review; new hash re-inspected",
}
...
def all_cases():
    ...
    impls = {**cases_f01.implementations(), **cases_f02.implementations(),
             ...
             **cases_f24.implementations(),
             **cases_f25.implementations(),
             **cases_f26.implementations(),
             **cases_f27.implementations(),
             ...
             **cases_f33.implementations(),
             **cases_f34.implementations()}
    specs = {}
    for mid in MODULES:
        for n in range(1, 5):
            cid = f"{mid}-M{n:02d}"
